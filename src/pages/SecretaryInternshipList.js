 ...
 */
  ...
  function Namefilter() {
    const getInternship = async () => {
      const q = query(internshipCollectionRef, where("stuName", "==", values.name))
  ...
  function Nofilter() {
    const getInternship = async () => {
      const q = query(internshipCollectionRef, where("stuNo", "==", values.numara))
      const data = await getDocs(q);
      setInternship(data.docs.map((doc) => ({ ...doc.data() })))
    }
    getInternship()
  ...
  function Onaylananlar() {
    const getInternship = async () => {
      const q = query(internshipCollectionRef, where("confirm", "==", "Onaylandı"))
      const data = await getDocs(q);
      setInternship(data.docs.map((doc) => ({ ...doc.data() })))
    }
    getInternship()
  ...
  function Reddedilenler() {
    const getInternship = async () => {
      const q = query(internshipCollectionRef, where("confirm", "==", "Onaylanmadı"))
      const data = await getDocs(q);
      setInternship(data.docs.map((doc) => ({ ...doc.data() })))
    }
    getInternship()
  ...
  async function TabletoPdf() {
    ...
    autoTable(doc, {
      startY: 30,
      head: [['Ö. Numara', 'Baslangıc Tarihi', 'Bitis Tarihi', 'Onay Durumu']],
        ...
        <Table.Body>{
          internships.map((intern) => (
            ...
            <Table.Row key={intern.id}>
              ...
              <Table.Cell><Link style={{ color: "green" }} to={`/secretaryPage/${id}/InternshipRequests/${intern.id}`}> Görüntüle</Link></Table.Cell>
            </Table.Row>
          ))
        }
